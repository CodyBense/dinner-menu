...
type RecipeTableModel struct {
	table table.Model
	focus bool
}

func (m RecipeTableModel) Init() tea.Cmd { return nil }

func (m RecipeTableModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	var cmd tea.Cmd
	switch msg := msg.(type) {
	case tea.KeyMsg:
		switch msg.String() {
		...
		case "r":
			recipe := NewRecipeTable()
			return recipe.Update(nil)
		case "c":
			return m, tea.Batch(tea.Println(m.table.SelectedRow()))
		}
	}
...
func (m RecipeTableModel) View() string {
...
func SetRecipeData() []table.Row {
	...
	recipes, err := sqlite.FindAllRecipes(db)
...
func NewRecipeTable() TableModel {
